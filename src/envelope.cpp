...
float EnvelopeZero::process(float sample_duration [[maybe_unused]])
{
    if (m_is_on)
    {
        return 1;
...
float EnvelopeADSR::process(float sample_duration)
{
    if (m_stage == Stage::OFF)
    {
        m_level_raw = 0;
    ...
    {
        m_level_raw += sample_duration / m_attack;
    ...
    {
        m_level_raw -= sample_duration / m_decay;
    ...
    {
        m_level_raw -= sample_duration / m_release;
