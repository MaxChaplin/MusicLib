...
namespace MusicLib
{
    ...
    {
    public:
        explicit VoiceFM2(Voice& carrier, Voice& modulator, Envelope env, float freq1 = 440, float freq2 = 440, float fm_level = 0, float vol = 1., unsigned int oversamping);
    ...
    {
    public:
        explicit VoicePD(Oscillator& m_osc, PhaseFunction& phase_func, Envelope env, float freq = 440, float vol = 1.);
    ...
    private:
        std::shared_ptr<Oscillator> m_osc;
        std::shared_ptr<PhaseFunction> m_phase_func;
        std::shared_ptr<Envelope> m_env;
        float m_freq;
        float m_phase;
        float m_vol;
